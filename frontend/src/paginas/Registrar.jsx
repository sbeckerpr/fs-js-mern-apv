...
const Registrar = () => {
  ...
  const handleSubmit = async e => {
    ...
    try {
      ...
      setAlerta({ msg: "Usuario creado exitosamente. Revisa el email", error: false});
    ...
    <>
      ...
      <div className='mt-20 md:mt-5 shadow-lg px-5 py-10 rounded-xl bg-white'>
        ...
        <form onSubmit={handleSubmit}>
          ...
          <input type="submit" value="Crear Cuenta" className="bg-indigo-700 p w-full py-3 px-10 rounded-xl text-white font-bold mt-5 hover:cursor-pointer hover:bg-indigo-800 md:w-auto" />
        </form>
          <nav className='mt-10 lg:flex lg:justify-between'>
            <Link className='block my-5 text-gray-500' to="/">¿Ya tienes una cuenta? Inicia Sesion</Link>
            <Link className='block my-5 text-gray-500' to="/olvide-password">Olvide mi password</Link>
          </nav>
      </div>
    </>
  )
